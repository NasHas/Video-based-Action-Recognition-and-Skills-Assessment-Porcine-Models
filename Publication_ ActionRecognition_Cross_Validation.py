...
def train(Y_trainImg, Y_trainLabel, Y_valImg, Y_valLabel, model,fold_no, n_epochs=EPOCHS, n_batch=BATCH_SIZE, image_path=img_path):
    ...
    for i in range(n_steps):
        ...
        if valCount % earlyStoppingVar == 0:  # Early stopping is triggered
            ...
            filename_loss = f'LossCurve_AR_fold{fold_no}.png'
            ...
            pyplot.xlabel('Epochs')
            filename_accuracy = f'AccuracyCurve_AR_fold{fold_no}.png'
            pyplot.savefig(filename_accuracy)
            pyplot.close()

            # Save losses and accuracies
            np.savetxt(f"train_loss_arr_AR_fold{fold_no}.csv", train_loss_arr, delimiter=",")
            np.savetxt(f"val_loss_arr_AR_fold{fold_no}.csv", val_loss_arr, delimiter=",")

            np.savetxt(f"acc_arr_train_AR_fold{fold_no}.csv", acc_arr_train, delimiter=",")
            np.savetxt(f"acc_arr_val_AR_fold{fold_no}.csv", acc_arr_val, delimiter=",")
        ...
        if (i + 1) % (bat_per_epo) == 0: # Summarize performance for every epoch
            ...
            if val_loss_arr[j-1] < best_val_loss:
                ...
                filename1 = f'Best_model_AR_fold{fold_no}.h5'
...
for train_index, val_index in kf.split(all_data):
    ...
    plot_model(model, to_file='CNNLSTM_AR.png', show_shapes=True,
