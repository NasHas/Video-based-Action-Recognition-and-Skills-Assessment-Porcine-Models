...
def generate_samples(img_name, label, n_samples, img_path):
    ...
    for j in range(len(X1)):
        for i in range(len(X1[1-1])):
            ...
            #print(f"Round {j + 1}, Image Name: {FullName}")
            ...
            elif loadedImgSize[1] == 480:  # the image is 640x480 # EXTRA FOR THE JIGSAW DATASET
                pass
...
def train(Y_trainImg, Y_trainLabel, Y_valImg, Y_valLabel, model, n_epochs=EPOCHS, n_batch=BATCH_SIZE,
    ...
    for i in range(n_steps):
        ...
        if valCount % earlyStoppingVar == 0:  # Early stopping is triggered
            ...
            filename_loss = 'LossCurve_SA.png'
            ...
            pyplot.xlabel('Epochs')
            filename_accuracy = 'AccuracyCurve_SA.png'
            pyplot.savefig(filename_accuracy)
            pyplot.close()

            # Save losses and accuracies
            np.savetxt("train_loss_arr_SA.csv", train_loss_arr, delimiter=",")
            np.savetxt("val_loss_arr_SA.csv", val_loss_arr, delimiter=",")

            np.savetxt("acc_arr_train_SA.csv", acc_arr_train, delimiter=",")
            np.savetxt("acc_arr_val_SA.csv", acc_arr_val, delimiter=",")
        ...
        if (i + 1) % (bat_per_epo) == 0: # Summarize performance for every epoch
            ...
            if val_loss_arr[j-1] < best_val_loss:
                ...
                filename1 = 'Best_model_Porcine_SA.h5'
...
# Build the model
model = get_model() #if you want to train from scratch
# load_model('Best_model_Porcine_SA.h5')
opt = Adam(learning_rate=0.0001)
model.compile(optimizer=opt, loss='categorical_crossentropy',metrics='categorical_accuracy')
model.summary()

plot_model(model, to_file='CNNLSTM_SA.png', show_shapes=True, show_layer_activations=True)
...
np.save('TrainLabelLSTM_SA.npy', Y_trainLabel_LSTM)
np.save('TestLabelLSTM_SA.npy', Y_testLabel_LSTM)
np.save('ValLabelLSTM_SA.npy', Y_valLabel_LSTM)
np.save('TrainImgLSTM_SA.npy', Y_trainImg_LSTM)
np.save('TestImgLSTM_SA.npy', Y_testImg_LSTM)
np.save('ValImgLSTM_SA.npy', Y_valImg_LSTM)
